...
let show_prompt = function () {
    let flag = prompt('请输入文件夹名字')
    if (flag != '') {
        return flag
    } else return show_prompt()
}

$new_folders.click(function () {
    let title = $('.breadmenu span').text()
    let new_name = show_prompt()
    for (let d in data) {
        if (title == data[d]['title']) {
            let newid = Math.max.apply(Math, Object.keys(data)) * 1 + 1
            data[newid.toString()] = {
                "id": newid,
                "pid": d * 1,
                "title": new_name,
            ...
            console.log(data)
            tools.getChild(data, d).forEach(item => item.checked = false);
            render(d * 1);
            createMenu(d);
        }
    }
